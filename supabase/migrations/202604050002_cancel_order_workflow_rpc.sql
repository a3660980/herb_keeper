...
begin
    ...
    if not found or v_current_status = 'canceled' then
        return;
    end if;
...
begin
    ...
    if v_current_status = 'canceled' then
        raise exception 'Canceled orders cannot be updated';
    end if;
...
grant execute on function public.update_order_with_items(uuid, uuid, timestamptz, text, jsonb) to authenticated, service_role;
...
begin
    ...
    if v_order_status = 'canceled' then
        raise exception 'Canceled orders cannot create shipments';
    end if;
...
begin
    ...
    if v_current_status = 'canceled' then
        raise exception 'Order already canceled';
    end if;
    ...
    if exists (
        ...
        raise exception 'Only orders without shipment history can be canceled';
    end if;
